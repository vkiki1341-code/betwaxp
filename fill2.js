...
async function resetAllLeaguesWeeks() {
  for (const league of LEAGUES) {
      if (league.code === 'en' || league.code === 'es' || league.code === 'it' || league.code === 'de') continue; // Skip Premier League, La Liga, Serie A, and Bundesliga
    for (let week = Math.min(10000, league.totalWeeks); week >= 1; week--) {
      const matches = [];
