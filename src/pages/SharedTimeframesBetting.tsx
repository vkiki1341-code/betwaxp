// --- Export current fixture state for Predictor sync ---
// This must be inside the component after selectedCountry is defined
// Usage: Call getCurrentBetXPesaFixture() from inside BetXPesa component
// (Removed duplicate implementation to fix error)
import React, { useEffect, useState, useRef } from "react";

// Define refetchOnSlotChange before using it
function refetchOnSlotChange() {
  // This function should trigger the same logic as your slot/timeframe change effect
  // You can call the same logic as in your useEffect for slot/timeframe changes
  // For example, you can trigger a state update or re-fetch admin outcomes here
  // If you want to keep the logic DRY, you can extract the code from the effect into this function
  // For now, this is a placeholder and should be filled with the actual logic as needed
}
...
// Helper to generate matchId using the actual week number (not index)
...
import { getMatchResultFromResultsTable } from "@/lib/getMatchResultFromResultsTable";
// Updated: getFinalScore now supports async fallback to match_results
async function getFinalScore(matchId: string, adminOutcomes: Record<string, any>, matchSimCache: Record<string, any>) {
  ...
  // Fallback: Try to get from match_results table
  const matchResult = await getMatchResultFromResultsTable(matchId);
  if (matchResult) {
    return {
      home: matchResult.homeGoals || 0,
      away: matchResult.awayGoals || 0,
      isAdminSet: true // treat as admin-set for display
    };
  }
  // UI warning for missing outcome
  if (typeof window !== 'undefined' && window.console) {
    console.warn(`[Betting UI] No admin outcome or match_results found for match_id: ${matchId}`);
  }
...
const getVisibleTimeSlots = (allSlots: Date[], currentIdx: number) => {
  const TIMEFRAMES_TO_SHOW = 100; // Show 100 timeframes (current + 99 upcoming)
...
const BetXPesa = () => {
  ...
  const totalFixtureSets = leagues.length; // e.g., 20 leagues
  const totalWeeks = 10000;
  // Track current fixture set and week (timeframe)
  const [fixtureSetIdx, setFixtureSetIdx] = useState(0); // 0-based
  const [currentTimeframeIdx, setCurrentTimeframeIdx] = useState(0); // 0-based week
  const [liveTimeframeIdx, setLiveTimeframeIdx] = useState(0);
  // Duplicate declaration removed to fix redeclaration error

  // Declare selectedCountry BEFORE any effect that uses it
  const [selectedCountry, setSelectedCountry] = useState(leagues[0].countryCode);

    // --- Attach current fixture state for Predictor sync ---
    // @ts-ignore
    React.useEffect(() => {
      (window as any).currentBetXPesaFixture = {
        get: () => ({
          week: currentTimeframeIdx + 1,
          country: selectedCountry,
          fixtureSetIdx: fixtureSetIdx,
          salt: (typeof getGlobalFixtureSalt === 'function') ? getGlobalFixtureSalt({ fixtureSetIdx }) : String(fixtureSetIdx),
          league: leagues.find(l => l.countryCode === selectedCountry)
        })
      };
    }, [currentTimeframeIdx, selectedCountry, fixtureSetIdx]);
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      const { data, error } = await supabase
        .from('match_results')
        .select('home_team, away_team, home_goals, away_goals, winner, is_final, updated_at')
  ...
  async function saveFinalMatchResult(matchId: string, homeGoals: number, awayGoals: number, winner: string) {
    try {
      ...
      const upsertPayload = {
        ...
        is_locked: true,
        match_duration: 90,
        week: Math.max(1, Math.min(week, 10000)),
        updated_at: now,
        created_at: now,
        goal_timings: '[]',
        result_version: 0,
      ...
      // Only log upsert payload for the current week to reduce log spam
      const globalSchedule = getGlobalSchedule();
      const nowIdx = Math.floor((getNowWithServerOffset() - globalSchedule.referenceEpoch) / (globalSchedule.matchInterval * 60000));
      const currentGlobalWeek = ((nowIdx % 36) + 1); // Assuming 36 weeks per season
      if (week === currentGlobalWeek) {
        console.log('match_results upsertPayload (current week):', JSON.stringify(upsertPayload));
      }
  ...
  // moved selectedCountry declaration above to fix use-before-declare error
  const [matchupsByTimeframe, setMatchupsByTimeframe] = useState({});
  // Removed matchResultsById: use only adminOutcomes for all outcome display
  ...
  // Define the MatchState type
  type MatchState = 'pre-countdown' | 'playing' | 'betting' | 'next-countdown';

  // Declare matchState and its setter BEFORE any effect that uses it
  const [matchState, setMatchState] = useState<MatchState>('pre-countdown');
  function parseMatchState(s: string): MatchState {
    switch (s) {
      case 'pre-countdown':
      case 'playing':
      case 'betting':
      case 'next-countdown':
        return s;
      default:
        console.warn(`Unknown matchState "${s}", defaulting to 'pre-countdown'`);
  ...
  useEffect(() => {
    ...
    return () => {
      if (unsubscribe) {
        ...
        // Use a robust salt for the initial fixture schedule
        const [fixtureSchedule, setFixtureSchedule] = useState(getFixtureSet(leagues[0], 0, getGlobalFixtureSalt({fixtureSetIdx: 0})));
    ...
    useEffect(() => {
      ...
      // CRITICAL: Sync with global system immediately on mount
      // This ensures ALL users see the SAME match regardless of when they enter
      const syncImmediately = async () => {
        if (didGlobalSyncRef.current) {
          // Prevent repeated sync loops across re-renders
          return;
        }
        didGlobalSyncRef.current = true;
        try {
          // Clear any cached timeframe data to prevent stale values
          localStorage.removeItem(SYSTEM_STATE_KEY);
          console.log('🔄 Starting global synchronization...');
          setWaitingGlobalState(true);
          
          // STEP 1: Ensure global schedule is synced from Supabase (source of truth)
          // Add timeout to prevent infinite loading
          const { ensureGlobalScheduleSynced, getGlobalSchedule } = await import('@/lib/matchScheduleService');
          ...
          let globalSchedule;
          try {
            globalSchedule = await Promise.race([
              ensureGlobalScheduleSynced(),
              timeoutPromise
            ]);
            console.log('✅ Schedule sync completed');
          } catch (timeoutError) {
            console.warn('⚠️ Supabase sync timed out, using local fallback');
            const { getGlobalSchedule: getLocalSchedule } = await import('@/lib/matchScheduleService');
            globalSchedule = getLocalSchedule();
          }
          
          // Validate schedule
          if (!globalSchedule || !globalSchedule.referenceEpoch) {
            throw new Error('Invalid schedule object');
          }
          
          console.log('✅ Global schedule loaded:', {
            referenceEpoch: new Date(globalSchedule.referenceEpoch).toISOString(),
            matchInterval: globalSchedule.matchInterval + ' minutes',
            currentTime: new Date(getNowWithServerOffset()).toISOString()
          });
          
          // CRITICAL: Log synchronization details for verification
          console.log('═══════════════════════════════════════════════════════');
          console.log('🌍 GLOBAL SYNCHRONIZATION ACTIVE');
          console.log('═══════════════════════════════════════════════════════');
          console.log('📍 Reference Epoch:', new Date(globalSchedule.referenceEpoch).toLocaleString());
          console.log('⏱️  Match Interval:', globalSchedule.matchInterval, 'minutes');
          console.log('🕐 Current Time:', new Date(getNowWithServerOffset()).toLocaleString());
          console.log('📊 Current Match Index:', Math.floor((getNowWithServerOffset() - globalSchedule.referenceEpoch) / (globalSchedule.matchInterval * 60000)));
          console.log('✨ All users worldwide are seeing THIS EXACT SAME MATCH');
          console.log('═══════════════════════════════════════════════════════');
          
          // STEP 2: Calculate current timeframe index and match state from global schedule
          // This ensures all users see the same match at the same time
          const now = new Date(getNowWithServerOffset());
          const timeSinceEpoch = now.getTime() - globalSchedule.referenceEpoch;
          const matchIntervalMs = globalSchedule.matchInterval * 60000;
          const currentIdx = Math.floor(timeSinceEpoch / matchIntervalMs);
          const weekIdx = ((currentIdx % totalWeeks) + totalWeeks) % totalWeeks;

          // Calculate where we are in the current match interval
          const msIntoCurrentMatch = timeSinceEpoch % matchIntervalMs;
          // Define phase durations (in ms)
          const preCountdownMs = PRE_COUNTDOWN_SECONDS * 1000;
          const matchSimMs = MATCH_SIM_SECONDS * 1000;
          const bettingWindowMs = BETTING_WINDOW_SECONDS * 1000;
          let matchStateToSet = 'pre-countdown';
          let countdownToSet = PRE_COUNTDOWN_SECONDS;
          let matchTimerToSet = 0;
          let bettingTimerToSet = BETTING_WINDOW_SECONDS;


          if (msIntoCurrentMatch < preCountdownMs) {
            // Still in pre-countdown
            matchStateToSet = 'pre-countdown';
            countdownToSet = Math.ceil((preCountdownMs - msIntoCurrentMatch) / 1000);
          } else if (msIntoCurrentMatch < preCountdownMs + matchSimMs) {
            // Match is playing
            matchStateToSet = 'playing';
            // Calculate the correct minute (0-90)
            matchTimerToSet = Math.floor(((msIntoCurrentMatch - preCountdownMs) / matchSimMs) * 90);
          } else if (msIntoCurrentMatch < preCountdownMs + matchSimMs + bettingWindowMs) {
            // Betting window
            matchStateToSet = 'betting';
            bettingTimerToSet = Math.ceil((preCountdownMs + matchSimMs + bettingWindowMs - msIntoCurrentMatch) / 1000);

            // SPECIAL HANDLING: If betting window is almost over, advance immediately
            if (bettingTimerToSet <= 2) {
              // Skip to next match instead of showing tiny betting window
              matchStateToSet = 'pre-countdown';
              countdownToSet = PRE_COUNTDOWN_SECONDS;
              // Calculate next match index
              const nextIdx = currentIdx + 1;
              const nextWeekIdx = ((nextIdx % totalWeeks) + totalWeeks) % totalWeeks;
              setCurrentTimeframeIdx(nextWeekIdx);
              setLiveTimeframeIdx(nextWeekIdx);
            }
          } else {
            // Next countdown or transition
            matchStateToSet = 'next-countdown';
          }

          setCurrentTimeframeIdx(weekIdx);
          setLiveTimeframeIdx(weekIdx);
          setMatchState(parseMatchState(matchStateToSet));
          // Only set countdown from global state, never locally
          setCountdown(countdownToSet);
          setMatchTimer(matchTimerToSet);
          setBettingTimer(bettingTimerToSet);
          setWaitingGlobalState(false);

          console.log('🕐 Current match index:', currentIdx, '→ week', weekIdx + 1, '(All users will see this same match)');
          console.log('✅ Global sync complete - all users synchronized');
          console.log('🎯 User who logged in at 1pm and user who logs in at 2pm will BOTH see match index:', currentIdx);
          console.log('💯 Perfect synchronization guaranteed!');
          
        } catch (err) {
          console.error('❌ Critical: Global sync failed:', err);
          
          // Fallback: Calculate from local time
          try {
            const { getGlobalSchedule } = await import('@/lib/matchScheduleService');
            const fallbackSchedule = getGlobalSchedule();
            const now = new Date(getNowWithServerOffset());
            const timeSinceEpoch = now.getTime() - fallbackSchedule.referenceEpoch;
            const currentIdx = Math.floor(timeSinceEpoch / (fallbackSchedule.matchInterval * 60000));
            const weekIdx = ((currentIdx % totalWeeks) + totalWeeks) % totalWeeks;
            ...
            console.warn('⚠️ Using fallback calculation - may not be in sync with other users');
          } catch (fallbackErr) {
...
React.useEffect(() => {
    ...
    async function fetchAndSubscribeAdminOutcomes() {
      ...
      fixtureSchedule.forEach((week, weekIdx) => {
        (week.matches || []).forEach((fixture, i) => {
          allMatchIds.push(getMatchId(selectedCountry, week.week - 1, i, fixture.home.shortName, fixture.away.shortName));
        });
      });
      if (allMatchIds.length === 0) return;
      // DEBUG: Print the batch of match IDs being requested for admin outcomes
      console.log('[ADMIN OUTCOME DEBUG] Fetching admin outcomes for matchIds:', allMatchIds, 'currentTimeframeIdx:', currentTimeframeIdx);
      const updateAdminOutcomes = async () => {
        const outcomes = await getFixtureOutcomesForWeek(allMatchIds);
        console.log('[ADMIN OUTCOME DEBUG] fetched outcomes:', outcomes);
        const map: Record<string, { homeGoals: number; awayGoals: number; winner?: string; homeTeam?: string; awayTeam?: string; updated_at?: string }> = {};
        ...
        console.log('[ADMIN OUTCOMES STATE UPDATED]', map);
      };
  ...
  // Get teams for selected country (always recalculate in render)
  const league = leagues.find(l => l.countryCode === selectedCountry);
  const teams = React.useMemo(() => {
    const l = leagues.find(lg => lg.countryCode === selectedCountry);
    return l ? l.teams : [];
  ...
  React.useEffect(() => {
    ...
    if (currentTimeframeIdx >= totalWeeks) {
      newSetIdx = (fixtureSetIdx + 1) % totalFixtureSets;
      newWeekIdx = 0;
    ...
    const loadGlobalTimeMatches = async (scheduleForLeague: any[], slotsList: Date[]) => {
      ...
      // For each slot, show all 9 matches for the current week, in fixture order
      const schedule = getGlobalSchedule();

      // Always use global week index from system state for match ID generation
      const globalWeekIdx = currentTimeframeIdx;
      console.log('[MATCHUP DEBUG] Using globalWeekIdx:', globalWeekIdx);
      slotsList.forEach((slot) => {
        const weekIdx = globalWeekIdx;
        const week = scheduleForLeague[weekIdx];
        const matches = week && week.matches ? week.matches : [];
        ...
        for (let matchIdx = 0; matchIdx < Math.min(matches.length, 9); matchIdx++) {
          ...
          // Always use week.week - 1 (0-based) for matchId, matching admin page
          const matchId = getMatchId(selectedCountry, week.week - 1, matchIdx, homeTeam.shortName, awayTeam.shortName);
          ...
          if (typeof window !== 'undefined' && window.console) {
            ...
            if (adminOutcomes && adminOutcomes[matchId]) {
              console.log('[MATCHID DEBUG][BETTING] matchId:', matchId, 'params:', matchIdParams, adminOutcomes[matchId]);
            } else {
              console.warn('[MATCHID DEBUG][BETTING] matchId:', matchId, 'params:', matchIdParams);
            }
          }
    ...
    loadGlobalTimeMatches(fixtureSchedule, slots);
    console.log('🌐 Global sync active - all users worldwide seeing same matches at ' + new Date().toISOString());
  }, [selectedCountry, currentTimeframeIdx, fixtureSetIdx, waitingGlobalState]);
  ...
  // Unified betting phase with atomic advancement
  React.useEffect(() => {
    if (matchState !== 'betting') return;
    let timer: any = null;
    const advanceToNextWeek = async () => {
      // CRITICAL: Prevent multiple concurrent advancements
      ...
      // Handle wrap-around
      if (newWeekIdx >= totalWeeks) {
        newSetIdx = (fixtureSetIdx + 1) % totalFixtureSets;
        newWeekIdx = 0;
        reshuffleTriggered = true;
      }
      // If reshuffling, ensure we always move to the next fixture set and week
      if (reshuffleTriggered) {
        try {
          // Delete all match results (must include a WHERE clause for Supabase/Postgres)
          const { error: deleteError } = await supabase.from('match_results').delete().not('match_id', 'is', null);
          if (deleteError) {
            console.error('❌ Error deleting match_results for new season:', deleteError);
          } else {
            console.log('✅ Cleared match_results for new season.');
          }
          // Delete all fixtures for all leagues (must include a WHERE clause)
          const { error: fixturesDeleteError } = await supabase.from('fixtures').delete().not('id', 'is', null);
          if (fixturesDeleteError) {
            console.error('❌ Error deleting old fixtures:', fixturesDeleteError);
          } else {
            console.log('✅ Cleared old fixtures for all leagues.');
          }
          // Generate a new robust salt for this season
          const fixtureSalt = `${Date.now()}_${Math.random().toString(36).slice(2, 10)}`;
          // Save the new salt and advance to the next fixture set and week in global system state
          ...
          // Update local state with new salt (for immediate UI update)
          // setFixtureSalt && setFixtureSalt(fixtureSalt); // Uncomment if you have a setFixtureSalt state
          console.log('✅ Generated new fixtureSalt for new season:', fixtureSalt);
        } catch (err) {
          console.error('❌ Exception during match_results/fixtures clear/reshuffle:', err);
        }
      }
      // Always advance to the next fixture set and week after reshuffle
      saveSystemStateToSupabase({
        currentWeek: newWeekIdx + 1,
        currentTimeframeIdx: newWeekIdx,
        fixtureSetIdx: newSetIdx,
        ...
        setLiveTimeframeIdx(newWeekIdx);
        setFixtureSetIdx(newSetIdx);
        setMatchState('pre-countdown');
        setMatchTimer(0);
        setBettingTimer(BETTING_WINDOW_SECONDS);
        setResultsSavedForPhase(false);
        console.log(`✅ [ADVANCE COMPLETE] Now at Week ${newWeekIdx + 1} (Set ${newSetIdx})`);
      });
    };
    ...
    if (bettingTimer > 0) {
      ...
      // If bettingTimer is already 0, advance immediately
      advanceToNextWeek();
    }
    return () => {
      if (timer) clearInterval(timer);
    };
  }, [matchState, bettingTimer, currentTimeframeIdx, fixtureSetIdx]);
  ...
  React.useEffect(() => {
    ...
    (async () => {
      try {
        console.log('[DEBUG] Fetching fixture_outcomes for matchIds:', matchIds);
        const { data, error } = await supabase
          .from('fixture_outcomes')
          .select('match_id, home_goals, away_goals, result, home_team, away_team, updated_at')
          .in('match_id', matchIds);
        console.log('[DEBUG] Supabase returned:', { data, error });
        if (error) {
          console.error('Failed to fetch fixture_outcomes:', error.message, error);
          return;
        }
        if (!data || data.length === 0) {
          console.warn('[DEBUG] No fixture_outcomes returned for matchIds:', matchIds);
        }
        const map: Record<string, { home_goals: number; away_goals: number; result: string; home_team: string; away_team: string; updated_at: string }> = {};
        (data || []).forEach((r: any) => {
          map[r.match_id] = {
    ...
    try {
      ...
      const formattedBets = betsToPlace.map(bet => {
        ...
        return {
          ...
          amount: bet.stake,
          odds: bet.odds,
          bet_type: bet.bet_type || bet.betType, // Use bet.betType if present (user selection)
        };
      });
      ...
      <div className="bg-gradient-to-r from-green-600/20 via-emerald-600/20 to-green-600/20 border-y border-green-500/30 py-2 px-4">
        <div className="max-w-7xl mx-auto flex items-center justify-center gap-3">
          ...
          <div className="hidden md:flex items-center gap-2 text-xs text-green-300/50">
            <span>Match #Week {((currentTimeframeIdx % totalWeeks) + 1)}</span>
            <span>•</span>
            ...
            <React.Fragment key={league.countryCode}>
              <button 
                onClick={() => {
                  ...
                  if (leagueIdx !== -1) {
                    setFixtureSchedule(getFixtureSet(leagues[leagueIdx], fixtureSetIdx, getGlobalFixtureSalt({fixtureSetIdx})));
                  }
              ...
              {showFixture && (
                <Dialog open={showFixture} onOpenChange={setShowFixture}>
                  <DialogContent className="max-w-4xl w-[95vw] sm:w-auto mx-auto overflow-y-auto max-h-[90vh] bg-gradient-to-br from-slate-900 to-slate-950 border-2 border-purple-500/30 text-white backdrop-blur-xl">
                    <DialogHeader>
                      <DialogTitle className="text-xl sm:text-2xl bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">🏆 League Fixtures (Week {currentTimeframeIdx + 1} of 10,000)</DialogTitle>
                    </DialogHeader>
                    ...
                    <div className="space-y-6 max-h-[500px] overflow-y-auto">
                      {leagues.filter(l => l.countryCode === selectedFixtureCountry).map(league => {
                        ...
                        const countryFixtureSchedule = leagueIdx !== -1 ? getFixtureSet(leagues[leagueIdx], fixtureSetIdx, getGlobalFixtureSalt({fixtureSetIdx})) : [];
                        // Only show current week and next 20 weeks for both admin and user
                        const startWeekIdx = currentTimeframeIdx;
                        const endWeekIdx = Math.min(countryFixtureSchedule.length, startWeekIdx + 21); // inclusive of current week
                        const visibleWeeks = countryFixtureSchedule.slice(startWeekIdx, endWeekIdx);
                        return (
                    ...
                    return filteredMatches.map((match, idx) => {
                      ...
                      if (currentTimeframeIdx < liveTimeframeIdx) {
                        ...
                        if (storedResult) {
                          ...
                          // Fallback: Simulated
                          const simResult = simulateMatch(match.id, 40, manualOutcome);
                ...
                <div className="bg-gradient-to-br from-slate-800/30 to-slate-900/30 p-6 rounded-xl mb-4 border-2 border-cyan-500/30 shadow-lg">
                  ...
                  {selectedTimeSlot && matchupsByTimeframe[selectedTimeSlot.toISOString()] && (() => {
                    ...
                    return filteredMatches.map((match, idx) => {
                      ...
                      // Use adminOutcomes for rightSection display
                      const manualOutcome = adminOutcomes[match.id];
                      let rightSection;
                      if (isPast && manualOutcome) {
                        rightSection = (
                          <div className="flex flex-col items-center gap-1 p-3 rounded-lg border border-green-500/40 bg-green-900/20 relative">
                            <div className="text-xs font-semibold text-green-300 flex items-center gap-1">FT</div>
                            <div className="text-lg sm:text-2xl font-bold text-green-400">
                              {manualOutcome.homeGoals} - {manualOutcome.awayGoals}
                            </div>
                            <div className="text-xs text-muted-foreground">
                              Winner: {manualOutcome.winner}
                            </div>
                          </div>
                        );
                      } else {
...
export function getCurrentBetXPesaFixture() {
